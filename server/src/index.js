...
import paymentsRouter, { stripeWebhookHandler } from './payments.js';
...
app.post('/scan', authenticate, async (req, res) => {
  ...
  const isPro =
    req.user.planTier === 'pro' && ['active', 'trialing'].includes(req.user.subscriptionStatus ?? '');
  if (!url || typeof url !== 'string') {
    return res.status(400).json({ error: 'Request body must include a valid url string.' });
  }

  try {
    const cooperationOptions = isPro ? normaliseCooperation(cooperation) : undefined;
    const allowFallback = isPro ? allowBrowserFallback !== false : false;
    const results = await scanWebsite(url, {
      cooperation: cooperationOptions,
