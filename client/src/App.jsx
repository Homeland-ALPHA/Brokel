...
function App() {
  ...
  const initialTokens = () => {
    ...
    try {
      const stored = window.localStorage.getItem('bl-auth');
      return stored ? JSON.parse(stored) : { accessToken: null, refreshToken: null };
    } catch (err) {
      console.warn('Failed to parse stored auth tokens', err);
      return { accessToken: null, refreshToken: null };
    }
  };
  ...
  const isAuthenticated = Boolean(authTokens.accessToken && user);
  const isPro =
    isAuthenticated && user && user.planTier === 'pro' && ['active', 'trialing'].includes(user.subscriptionStatus ?? '');
  ...
  const checkoutStatus = checkoutMeta.status;
  ...
  const storeAuthTokens = (tokens) => {
    if (tokens?.accessToken && tokens?.refreshToken) {
      const stored = { accessToken: tokens.accessToken, refreshToken: tokens.refreshToken };
      setAuthTokens(stored);
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('bl-auth', JSON.stringify(stored));
      }
    } else {
      setAuthTokens({ accessToken: null, refreshToken: null });
      if (typeof window !== 'undefined') {
        window.localStorage.removeItem('bl-auth');
      }
    }
  };
  ...
  const clearAuth = () => {
    setUser(null);
    storeAuthTokens({ accessToken: null, refreshToken: null });
    setAuthMode('login');
    setAuthForm({ email: '', password: '', fullName: '' });
    setAuthError('');
  };

  const authHeaders = () => (authTokens.accessToken ? { Authorization: `Bearer ${authTokens.accessToken}` } : {});
  ...
  const refreshTokens = async () => {
    ...
    try {
      ...
      const { tokens, user: refreshedUser } = response.data;
  ...
  const performAuthorizedPost = async (url, data, config = {}) => {
    try {
      ...
      if (error.response?.status === 401 && authTokens.refreshToken) {
        try {
          await refreshTokens();
          return await axios.post(url, data, {
            ...config,
  ...
  const fetchAccount = async (withRetry = true) => {
    ...
    try {
      ...
      if (withRetry && error.response?.status === 401 && authTokens.refreshToken) {
        try {
          await refreshTokens();
  ...
  useEffect(() => {
    if (checkoutStatus === 'success' && authTokens.accessToken) {
      fetchAccount(false);
      setBillingMessage('Subscription activated. Real-time monitoring is now unlocked.');
    } else if (checkoutStatus === 'canceled') {
      setBillingMessage('Checkout was cancelled. You can resume whenever you are ready.');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [checkoutStatus, authTokens.accessToken]);
  ...
  const handleAuthSubmit = async (event) => {
    ...
    try {
      ...
      if (authMode === 'register') {
        payload.fullName = authForm.fullName.trim() || undefined;
      }
  ...
  const handleModeSwitch = (mode) => {
    ...
    setAuthForm((previous) => ({
      ...
      password: '',
      fullName: mode === 'register' ? previous.fullName : '',
    }));
  };
  ...
  const handleScan = async (event) => {
    ...
    if (!isAuthenticated) {
      setError('Please sign in to run a scan.');
      return;
    }
    ...
    try {
      let response;
      try {
        response = await performAuthorizedPost(`${API_BASE_URL}/scan`, {
          ...payloadBase,
      ...
      if (requestError.response?.status === 401) {
        ...
        clearAuth();
      } else if (requestError.response?.data?.reason === 'browser-required') {
        setError('Browser-protected pages require the Pro plan with headless scans.');
        if (Array.isArray(requestError.response?.data?.partialFindings)) {
          setResults(requestError.response.data.partialFindings);
        }
      } else {
  ...
  const handleUpgrade = async () => {
    setBillingMessage('');
    if (!isAuthenticated) {
      setBillingMessage('Please sign in to upgrade.');
      return;
    }
    setCheckoutLoading(true);
    try {
      const response = await performAuthorizedPost(`${API_BASE_URL}/api/payments/checkout-session`, {});
      if (response.data?.url) {
        window.location.href = response.data.url;
        return;
      }
      setBillingMessage('Unable to start checkout.');
    } catch (error) {
      if (error.response?.status === 401) {
        clearAuth();
        setBillingMessage('Your session expired. Please sign in again.');
      } else {
        const detail = error.response?.data?.error ?? 'Unable to start checkout.';
        setBillingMessage(detail);
      }
    } finally {
  ...
  const handleManageBilling = async () => {
    setBillingMessage('');
    if (!isAuthenticated) {
      setBillingMessage('Please sign in to access billing.');
      return;
    }
    if (!user?.stripeSubscriptionId) {
      setBillingMessage('Billing portal becomes available after you start a subscription.');
      return;
    }
    setBillingLoading(true);
    try {
      const response = await performAuthorizedPost(`${API_BASE_URL}/api/payments/portal-session`, {});
      if (response.data?.url) {
        window.location.href = response.data.url;
        return;
      }
      setBillingMessage('Unable to open the billing portal.');
    } catch (error) {
      if (error.response?.status === 401) {
        clearAuth();
        setBillingMessage('Your session expired. Please sign in again.');
      } else {
        const detail = error.response?.data?.error ?? 'Unable to open the billing portal.';
        setBillingMessage(detail);
      }
    } finally {
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 py-12">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col items-start gap-2 text-sm text-slate-300 lg:items-end">
              {isAuthenticated ? (
                <>
                  <span className="text-xs uppercase tracking-widest text-slate-500">Signed in</span>
                  <span className="text-slate-100">{user.email}</span>
                  <span className="text-xs text-slate-400">
                    Plan:{' '}
                    <span className={isPro ? 'text-emerald-300 font-semibold' : 'text-amber-300'}>
                      {isPro ? 'Pro' : 'Free'}
                    </span>
                  ...
                  <button
                    ...
                    className="rounded-md border border-slate-700 px-4 py-2 text-xs text-slate-200 transition hover:border-slate-500 hover:text-slate-100"
                  >
                    Sign out
                  </button>
                </>
                ...
                <button
                  ...
                  className="rounded-md border border-emerald-400/60 px-4 py-2 text-xs font-semibold text-emerald-200 transition hover:bg-emerald-500/10"
                >
          ...
          <div className="grid gap-10 lg:grid-cols-[2fr,1fr] lg:items-start">
            ...
            {!isPro && <AdSlot slot="0000000000" className="max-lg:order-first" />}
          </div>
        </div>
      </header>
      <main id="mainContent" className="mx-auto flex w-full max-w-6xl flex-col gap-16 px-6 py-14">
        <section id="account" className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-lg shadow-slate-950/20">
          {isAuthenticated ? (
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="space-y-2">
                <h2 className="text-xl font-semibold text-emerald-200">Welcome back</h2>
                <p className="text-sm text-slate-300">Manage your scans and billing for <span className="text-slate-100">{user.email}</span>.</p>
                <p className="text-xs uppercase tracking-widest text-slate-500">
                  Current plan:{' '}
                  <span className={isPro ? 'text-emerald-300 font-semibold' : 'text-amber-300'}>
                    {isPro ? 'Pro ($100/mo)' : 'Free'}
                  </span>
                  {user?.subscriptionStatus ? ` · ${user.subscriptionStatus}` : ''}
                </p>
              </div>
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={handleLogout}
                  className="rounded-md bg-slate-800 px-4 py-2 text-sm font-semibold text-slate-200 transition hover:bg-slate-700"
                >
                  Sign out
                </button>
              </div>
            ...
            <div className="grid gap-6 md:grid-cols-[1.1fr,1fr]">
              <div className="space-y-3">
                <h2 className="text-2xl font-semibold text-emerald-200">Create your BrokenLink AI account</h2>
                <p className="text-sm text-slate-300">Save scan history, unlock continuous monitoring, and manage billing from a single dashboard.</p>
                <div className="flex gap-2 text-xs uppercase tracking-widest text-slate-500">
                  <button
                    type="button"
                    onClick={() => handleModeSwitch('login')}
                    className={`rounded-full px-3 py-1 ${authMode === 'login' ? 'bg-emerald-500/20 text-emerald-200' : 'bg-slate-800 text-slate-400'}`}
                  >
                    Sign in
                  </button>
                  <button
                    ...
                    className={`rounded-full px-3 py-1 ${authMode === 'register' ? 'bg-emerald-500/20 text-emerald-200' : 'bg-slate-800 text-slate-400'}`}
                  >
                    Create account
                  </button>
                </div>
              </div>
              <form className="grid gap-4 rounded-xl border border-slate-800 bg-slate-900/60 p-5" onSubmit={handleAuthSubmit}>
                <label className="text-sm text-slate-200" htmlFor="auth-email">
                  Email
                  <input
                    id="auth-email"
                    type="email"
                    ...
                    onChange={handleAuthInput('email')}
                    required
                  />
                </label>
                {authMode === 'register' && (
                  <label className="text-sm text-slate-200" htmlFor="auth-name">
                    Full name
                    <input
                      id="auth-name"
                      type="text"
                      ...
                      onChange={handleAuthInput('fullName')}
                    />
                  </label>
                )}
                <label className="text-sm text-slate-200" htmlFor="auth-password">
                  Password
                  <input
                    id="auth-password"
                    type="password"
        ...
        <section id="billing" className="flex flex-col gap-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="max-w-2xl space-y-3">
              <h2 className="text-2xl font-semibold text-emerald-200">Upgrade to BrokenLink AI Pro</h2>
              <p className="text-sm text-emerald-100/80">
                Remove sponsored modules, schedule continuous scans, and receive proactive alerts when critical journeys break.
              </p>
              {checkoutStatus === 'success' && (
                <p className="rounded-md bg-emerald-500/10 p-3 text-sm text-emerald-200">
                  Thanks for upgrading. Real-time monitoring is now active for this browser session.
                </p>
              )}
              {checkoutStatus === 'canceled' && (
                <p className="rounded-md bg-amber-500/10 p-3 text-sm text-amber-200">
                  Checkout was canceled. You can restart the upgrade whenever you are ready.
                </p>
              )}
              {!isAuthenticated && (
                <p className="rounded-md bg-amber-500/10 p-3 text-xs text-amber-200">Sign in to upgrade or manage billing.</p>
              )}
              {isAuthenticated && isPro && (
                <p className="rounded-md bg-emerald-500/10 p-3 text-xs text-emerald-200">Pro monitoring is active. Billing controls stay available below.</p>
              )}
              {billingMessage && (
                <p className="rounded-md bg-rose-500/10 p-3 text-sm text-rose-200">{billingMessage}</p>
              )}
            </div>
            <div className="flex flex-col gap-3 md:flex-row md:items-center">
              <button
                ...
                className="rounded-md bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-900 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {checkoutLoading ? 'Redirecting...' : 'Upgrade for $100/mo'}
              </button>
              <button
                type="button"
                onClick={handleManageBilling}
                disabled={!isAuthenticated || !user?.stripeSubscriptionId || billingLoading}
                className="rounded-md border border-emerald-400/60 px-4 py-2 text-sm font-semibold text-emerald-200 transition hover:bg-emerald-500/10 disabled:cursor-not-allowed disabled:opacity-60"
              >
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
            ...
            <dl className="mt-6 space-y-3 text-sm text-slate-300">
              ...
              <div>
                ...
                <dd>60 East 42nd Street, Suite 1201, New York, NY 10165</dd>
              </div>
            </dl>
          </div>
